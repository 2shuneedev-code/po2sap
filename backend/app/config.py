...
class Settings(BaseSettings):
    ...
    # 추론 깊이. **출력 토큰(=비용)을 가장 크게 좌우하는 손잡이다.**
    # 2026-09-21 실측: 발주서 1부에 출력 62,588 토큰 — 전체 비용의 65% 였다.
    # Opus 5 는 thinking 이 기본 ON 이고 그 토큰이 출력으로 과금된다.
    #
    # 추출은 추론이 깊게 필요한 작업이 아니라 기본을 medium 으로 둔다.
    # low  가장 쌈 — 단순 양식에서 먼저 시도해 볼 것
    # high 기본값이었던 값. 품질이 안 나올 때만 되돌린다
    #
    # **thinking 을 끄지는 않는다.** Opus 5 에서 끄면 도구 호출을 본문 텍스트에
    # 써버리는 실패가 알려져 있다 — 오류 없이 조용히 추출이 빈다.
    llm_effort: str = "medium"      # low | medium | high | xhigh | max

    # ── 마스터 편집 잠금 ───────────────────────────────────
    # 브랜드 매핑은 **서버 디스크의 masters/ 파일을 고친다.** 한 사람이 고치면
    # 그 서버를 쓰는 모두의 판정이 그 자리에서 바뀐다. 그래서 암호를 받는다.
    #
    # 기본값이 빈 문자열이고, 그때는 **편집이 잠긴다**(조회는 된다).
    # EAI_ENDPOINT 와 같은 이유다 — .env 를 깜빡한 채 띄웠을 때 열려 있는 것보다
