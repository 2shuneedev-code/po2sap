...
- 전송 필드를 현행 36개에서 **미사용·불필요한 것을 빼고 약 15개로 줄인다**
- **특정 경로에 `SALES ORDER` 템플릿을 저장**해 두고 **불러와서 사용**한다
- 템플릿의 **2행이 필드명**이다. **필드명으로 움직인다** (열 위치가 아니라)
- **조건이 없으면 그냥 공란으로 둔다**
...
## 2-B. 위 요구사항에서 아키텍트가 먼저 정해야 할 것

1. **엑셀 템플릿이 최종 산출물인가, EAI 전송과 병행인가.**
   ③ 은 산출물을 "엑셀 템플릿 채우기"로 읽히는데, 현행 설계의 종착점은
   `EAI(HTTPS/JSON) → SAP CBO` 다. 둘 중 하나인지 둘 다인지에 따라
   `transport/` 와 `mapping/` 의 위치가 갈린다. **사장님께 확인할 것.**
2. **필드 36 → 15 의 확정 목록.** `_base/sap_defaults.yaml` 이 SSOT 이므로
   거기서 빼는 것이 맞다. 무엇을 뺄지는 현업 확인이 필요하다.
3. **템플릿 파일의 위치와 Git 포함 여부.** 생성물이 아니라 입력물이므로
   `masters/templates/` 같은 추적 대상이 자연스럽다 (거래처마스터.xlsx 와 다름).
