...
```
- 🔴 error 가 하나라도 있으면 **HTTP 409** — 한 건도 나가지 않는다
- 요청의 값도 §6.1 과 같은 규칙으로 서버 스냅샷에 병합된다. 전송 직전이라고
  검증을 건너뛰지 않는다 — 여기가 마지막 방어선이다
- 재전송은 같은 API 재호출. `SENT` 배치도 다시 보낼 수 있고 응답에 `"resend": true`
  가 붙는다. `SENDING` 중에는 409

### 7.1 성공·실패 판정

**EAI 응답 본문 규격이 아직 확정되지 않았다.** 그래서 판정을 HTTP 상태 코드에만 건다.

| 응답 | 판정 | 재시도 |
|---|---|---|
| 2xx | 성공 | — |
| 4xx | 영구 실패 | **안 한다** — 같은 요청은 또 거부된다 |
| 5xx · 타임아웃 · 연결 실패 | 일시 실패 | `EAI_RETRIES` 회 (1·2·4초 백오프) |

응답 본문은 해석하지 않고 **감사 로그에 발췌를 남긴다.** 실제 트래픽이 쌓이면
본문으로 성공/부분실패를 구분하는 규격을 확정하고 판정을 추가한다.

> ⚠ **알려진 한계**: 지금은 "HTTP 200 인데 본문에 실패가 적힌" 경우를 잡지 못한다.
> EAI 담당과 응답 규격을 확정해야 닫히는 구멍이다.

### 7.2 대용량 분할

`EAI_MAX_ROWS_PER_REQUEST` 가 0보다 크면 그 수만큼 나눠 보낸다. 앞 묶음이 성공하고
뒤가 실패하면 `status: SEND_FAILED` 에 `sent_rows` 는 **성공한 만큼**이 된다.
재전송은 처음부터 다시 보낸다 — "중복 전송 무해"(design D7) 전제에 기대는 부분이다.

> ⚠ **그 전제가 아직 검증되지 않았다.** CBO 업서트 키가 확정되지 않아서, 키가 없으면
> 재전송이 덮어쓰기가 아니라 **중복 적재**가 된다. 감사 로그가 `send` 와 `resend` 를
> 구분해 남기므로 사후 추적은 가능하다.
...
| 운영 | `https://eai-prd…` — **이관 시 `.env` 만 바꾼다** |
| 로컬 | `http://127.0.0.1:9000/po2sap/order` (`scripts/mock_eai_server.py`) |

**HTTPS 가 아니면 보내기 전에 거부한다 (400).** 평문 `http` 는 루프백에서만 허용한다.
