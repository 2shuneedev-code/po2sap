...
> 원칙 3 의 "읽기"에는 **날짜를 `YYYYMMDD` 8자리로 적는 표기 변환**이 포함된다.
> 근거와 경계는 §4.2.1 에 있다. 그 외의 값 변환·코드 결정은 여전히 마스터의 몫이다.
...
- **날짜 성격의 키**(`po_date`, `delivery_date`, `ship_by_text` 중 날짜, `extra.*` 의 날짜)는
  **항상 `YYYYMMDD` 8자리 문자열**로 담는다 (§4.2.1).
...
extraction:
  ...
  hints: |
    (자연어로 문서 구조를 설명. 어떤 라벨 아래 어떤 값이 있는지,
     혼동하기 쉬운 컬럼이 무엇인지. 값 결정은 지시하지 않는다.)
```

**규칙**: `hints` 는 **문서의 어디에 무엇이 적혀 있는지**만 설명한다.
"브랜드를 SAP 코드로 바꿔라", "출하처를 KUNNR2 로", "이 값은 우리 쪽에서 뭐로 취급한다" 같은
**값 결정 지시를 쓰지 않는다.** 값 결정은 전부 `tables` / `rules` / `fields` 의 몫이다
(`CLAUDE.md` G3).

**유일한 예외는 날짜 표기다 — §4.2.1.**

#### 4.2.1 예외 — 날짜는 추출 단계에서 `YYYYMMDD` 8자리로 뽑는다 ★

**날짜 값에 한해** `hints` 에 "`YYYYMMDD` 8자리 문자열로 추출하라" 는 **표기 지시**를 쓴다.
원문이 `2/17/26` 이든 `2026.05.11` 이든 `2026-05-18` 이든, 추출 단계가 8자리로 만들어
`header.po_date` / `line.delivery_date` 등에 담는다. **엔진은 날짜를 다시 파싱하지 않는다.**

##### 왜 이것이 G3(원칙 3) 위반이 아닌가

| | 브랜드·출하처 코드 — **여전히 금지** | 날짜 표기 — **허용** |
|---|---|---|
| 결정에 필요한 것 | 우리 회사의 **업무 규칙표** (HERTEL→38) | 없음. 값은 이미 문서에 적혀 있다 |
| 하는 일 | 문서에 **없는 값을 새로 결정**한다 | 같은 값을 **다른 표기로 옮겨 적는다** |
| 규칙이 바뀌면 | 매핑표를 고쳐야 한다 (변경 관리·감사 대상) | 바뀔 것이 없다 (달력은 고정) |
| 검증 방법 | 결정표의 어느 행이 왜 골라졌는지 설명이 필요 | 원문과 1:1 대조하면 맞고 틀림이 즉시 보인다 |

G3 가 막으려는 것은 **감사 불가능한 업무 판단** — 어느 표의 어느 행이 왜 선택됐는지
재현·설명되지 않는 결정이다. 날짜의 표기 변환에는 결정표가 없고, 선택지도 없고,
검수 화면에서 원문과 나란히 놓으면 사람이 바로 확인한다. 따라서 G3 가 지키려는 것을
훼손하지 않는다.

오히려 **정확도 면에서 추출 단계가 낫다.** `2/17/26` 같은 표기는 월/일 순서와 두 자리
연도를 문서 맥락 — 거래처 국적, 발주일과 납기일의 선후 관계, 문서 내 다른 날짜 — 으로
판단해야 한다. 이 맥락을 가진 것은 문서를 통째로 읽는 추출 단계뿐이다. 엔진의 정규식
휴리스틱은 같은 맥락을 갖지 못한 채 구분자만 보고 추측하게 되고, 틀렸을 때 조용히 틀린다.

##### 예외의 경계 — 이것만 허용된다

| | |
|---|---|
| ✅ 허용 | 날짜 값을 `YYYYMMDD` 8자리로 **표기만 바꿔** 담는 것 |
| 🚫 금지 | 날짜를 **근거로 한 판단** — 납기 지연 여부, 분기/시즌 코드, 날짜 비교에 따른 분기.<br>이런 것은 전부 `tables` / `rules` / `fields.expr` 이 한다 |
| 🚫 금지 | **날짜가 아닌 값**의 형식·코드 변환 (브랜드·출하처·통화·단위·수량·금액 표기 등).<br>수량·금액 표기는 `format: integer` / `decimal3`(§4.6.1)가 한다 |

> **"날짜 하나를 8자리로 적는 것"까지가 추출의 일**이고,
> **"그 날짜로 무엇을 할지"는 규칙의 일**이다.

##### 안전망

⑦ VALIDATE 는 `field_specs.type: date`(§4.10) 인 필드의 최종 값이 **8자리 숫자인지** 검사하고,
아니면 **경고**를 붙인다(빈 문자열은 정상 — 경고 없음). 전송을 막지는 않는다(원칙 2).
추출이 지시를 어겨도 조용히 넘어가지 않게 하는 장치다.
...
MATNR:
  ...
  format: integer              # §4.6.1 의 이름 하나. 최종 값에 한 번 적용
  default: ""                  # 최종 폴백
...
- 필드 1개의 **값 결정 순서**: `from` 평가 → 비었으면 `fallback` → 비었으면 `default`
  → **`format` 적용** → ⑦ VALIDATE(`required`, `max_len`, 날짜 8자리).
  즉 `format` 은 **항상 마지막**이고, 필드당 **하나만** 쓴다.
- **날짜 필드에는 `format` 을 쓰지 않는다.** 값은 이미 `YYYYMMDD` 로 들어온다(§4.2.1).
  `{ from: doc, path: header.po_date }` 처럼 그대로 싣고, 검사는 ⑦ VALIDATE 가 한다.
...
- **날짜 포맷 함수는 없다.** 날짜는 ① EXTRACT 가 `YYYYMMDD` 로 내놓고(§4.2.1),
  ⑦ VALIDATE 가 8자리 여부만 검사한다. 엔진이 원문 날짜를 파싱하는 경로는 존재하지 않는다.
  (v1.2 에 있던 `date_yyyymmdd` 는 v1.3 에서 폐지됐다 — §8)
...
> 날짜 변환 함수는 화이트리스트에 **없다.** 식 안에서 날짜를 쓸 때는
> `header.po_date` 를 그대로 붙인다 — 이미 8자리다(§4.2.1).
...
```yaml
# (A) format — 문서의 수량을 그대로 필드에 싣되 표기만 정리한다
KWMENG: { from: doc, path: line.quantity, format: integer }

# (B) expr — 변환한 단가를 다른 조각과 붙여야 하므로 format 으로는 불가능하다
IHREZ_E:
  from: expr
  expr: 'join(" ", [decimal3(line.unit_price), header.currency_text])'
```

(B)를 `format` 으로 쓸 수 없는 이유: `format` 은 조합이 **끝난 뒤** 전체 문자열에 걸리므로
`"8.240 JPY"` 전체를 숫자로 해석하려 들어 실패한다.
...
> `type: date` 인 필드는 ⑦ VALIDATE 가 **8자리 숫자인지**만 검사한다(§4.2.1).
> 거래처 파일에 날짜 `format` 을 따로 달 필요가 없다 — 검사 기준은 이 파일 하나다.
...
2. meta 채우기
3. extraction.hints 작성 (샘플 발주서 보며 문서 구조 설명, 날짜는 YYYYMMDD 지시)
...
| 12 | `field_specs.type: date` 인 필드에 `format` 이 달려 있는가 (§4.6.1) | 경고 |

> 12 는 폐지된 날짜 파싱 경로가 되살아나는 것을 막는다. `date_yyyymmdd` 처럼
> 없는 이름을 쓰면 검사 2·4 에서 **오류**로 걸린다.
...
| v1.3 | 2026-09-11 | **날짜 처리 주체를 엔진 → 추출 단계로 이동** (사용자 결정: "날짜는 AI API 가 알아서 잘 읽고 스프레드시트에는 캐릭터 8 로 YYYYMMDD 로 입력하면 됨").<br>· §4.2 규칙 문구를 "값 결정 지시 금지"로 정리하고 **§4.2.1 신설** — 날짜에 한해 `hints` 의 `YYYYMMDD` 8자리 표기 지시 허용. G3 비위반 근거(업무 규칙표가 필요한 **값 결정** vs 문서에 적힌 값의 **표기 이동**)와 예외 경계(날짜 기반 판단·날짜 외 값 변환은 여전히 금지) 명시<br>· §4.6.1 **`date_yyyymmdd` 포맷 폐지** — 슬래시=미국식, 두 자리 연도=20xx 등 휴리스틱 서술 삭제<br>· §4.7 화이트리스트에서 `date_yyyymmdd` 제거, §4.7.1 예시를 `integer`/`decimal3` 기반으로 교체<br>· §4.10 `type: date` → ⑦ VALIDATE 의 **8자리 숫자 검사** 기준으로 정식화 (거래처별 날짜 `format` 불필요)<br>· §0 원칙 3 · §3.1 표준 키 · §4.6 값 결정 순서 · §5 절차에 날짜 규약 반영<br>· §7 검증 항목 12 추가 |
