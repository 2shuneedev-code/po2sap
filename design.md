...
>
> 버전 **v1.2** · 2026-09-21 · §3.3/§3.4 재작성 (대용량 발주서 분할 추출) · 실측치 반영 · 스캔본 경로 확정
>
> ### 이 문서의 범위
...
| D13 | **분할은 텍스트 문서에만 적용한다** | 스캔본은 줄 번호가 없어 나눌 수 없다. 단일 호출 + 한도 초과 시 명시적 거부 (§3.3.6) |
...
> 품목 1,226줄이었고, 한 번에 담으면 출력만 40만 토큰이 넘어 타임아웃으로 실패했다.
...
| 문서 분할 | **문서 1건 ≠ 호출 1회.** OUTLINE 1회 + LINES N회 — §3.3.2~3.3.4. 단 스캔본 제외(§3.3.6) |
...
**추출 스키마의 키 이름과 값 포장은 `masters/SCHEMA.md` §3.1·§3.2 가 단일 원천이다.**
여기에 다시 적지 않는다. 키를 바꿀 때는 그쪽을 먼저 고치고
...
크면 `max_tokens` 안에 몇십 줄밖에 안 들어간다.

실측 기준 문서는 MSC `P8033553.HTM` 이다.

| 항목 | 실측 |
|---|---|
| 전처리 텍스트 | 133,741자 |
| 품목 행 | **1,226줄** = 상단 요약표 **405** + Shipment 블록 안 **821** |
| Shipment 블록 | **4개** (`001 ELK` · `002 HBL` · `003 RNO` · `004 ATL`) |
| 품목 1줄의 출력 | 약 1,260자 ≈ 350~400토큰 |

1,226줄 × 375토큰 ≈ **46만 토큰**. `LLM_MAX_TOKENS=16000` 에는 약 40줄이 들어간다.
...
| 3 | 빈 필드 `null` 반복 | 품목 줄의 값 필드를 `required` 에서 빼고 **없으면 키를 생략** | 빈 필드 1개당 4줄이 0줄 |
| 4 | 요약표 품목 | `split.by` 가 `none` 이 아니면 **상단 요약표를 품목으로 읽지 않는다** (SCHEMA §2.1-2) | 추출 대상이 **1,226 → 821줄** (405줄, 33% 감소) |
...
| 그래도 실패한 청크 | **전체 실패로 만들지 않는다.** 나머지 결과를 살리고, 그 청크가 속한 오더 단위의 전 행에 🔴 `CHUNK_FAILED` 를 붙인다 → 검수 화면에는 보이고 전송은 막힌다. **메시지에 빠진 줄 범위를 적는다** |
...
| **스캔본에서 절단** | 쪼갤 수단이 없다. **즉시 명시적 실패** (§3.3.6) |
...
#### 3.3.6 스캔본(텍스트 레이어 없음) — 분할하지 않는다 ★

**줄 번호가 없는 문서는 나눌 수 없다.** 앵커도 청크 경계도 전처리 텍스트의
줄 번호에 기대고 있는데, 스캔본은 PDF 원본을 그대로 모델에 넘기므로
우리 손에 줄이 없다.

| 항목 | 결정 |
|---|---|
| 경로 | **단일 호출.** OUTLINE/LINES 2패스를 쓰지 않는다 |
| 스키마 | `src` 를 **필수에서 뺀 변형**을 쓴다. 대신 `page` 를 **모델이 준다** — 줄이 없을 때 위치를 가리킬 유일한 수단이다 |
| 근거 검증 | 앵커 대조를 **건너뛴다**(대조할 원문이 없다). `confidence` 임계 + 합계 교차검증만 남는다 |
| 한도 초과 | 페이지 수가 `extraction.page_limit` 을 넘거나 `stop_reason=max_tokens` 가 나면 **쪼갤 수단이 없다.** 조용히 잘린 결과를 내보내지 않고 **명시적으로 실패**한다 — "품목이 많은 스캔 발주서는 처리할 수 없습니다. 거래처에 텍스트 PDF 를 요청하세요." |

> `page` 를 모델에게 맡기는 것은 §3.1 의 "페이지 번호 계산은 안 시킴"에 대한
> **명시적 예외**다. 텍스트 경로에서는 코드가 `src` 로 역산하고, 스캔본에서만
> 모델이 준다. 두 경로가 같은 `ExtractedValue.page` 를 채우되 출처가 다르다.

**현재 이 경로를 쓰는 거래처는 없다.** `masters/` 의 전 거래처·프로필·템플릿이
`extraction.input: text` 다(2026-09-21 확인). 따라서 위는 **동작을 보장하기 위한
최소 처리**이고, 품목이 많은 스캔 발주서를 실제로 받게 되면 그때
**페이지 단위 앵커**(`page` + 페이지 내 순번)를 도입한다 — 후속 과제다.

> 관련해 지금 코드에 조용한 함정이 하나 있다. `input: text` 인데 실제로는
> 텍스트 레이어가 없는 PDF 가 들어오면, `use_text` 가 무조건 참이 되어
> **거의 빈 텍스트를 보내고 빈 결과를 받는다.** `input: text` + `has_text_layer=False`
> 는 위 "명시적 실패"로 보내야 한다.
...
| 판정 | 방법 | 결과 |
|---|---|---|
| `exact` | 공백·대소문자 정규화 후 포함. **숫자 경계를 지킨다** — 값이 숫자로 시작하거나 끝나면 그 자리에 이웃 숫자가 붙어 있으면 안 된다 | 통과 |
| `numeric` | 양쪽을 `Decimal` 로 읽어 수치 비교. 쉼표·통화기호를 걷어내고 **유럽식 표기(`1.250,00` · `12,50`)도 받는다** | 통과 |
| `date` | `value` 를 그 줄에 나올 법한 표기들로 되돌려 대조. **`M/D` · `D/M` · `Y/M/D` 순서와 월 이름 표기를 모두 만든다** (`2/17/26` · `17/2/26` · `2026-02-17` · `20260217` · `17-Feb-26` · `Feb 17, 2026` …) | 통과 |
| `fuzzy` | 토큰 겹침 비율이 임계 이상 | 🟡 `EVIDENCE_WEAK` |
| `miss` | 그 밖 | 🔴 `EVIDENCE_NOT_FOUND` |

> **숫자 경계를 지키는 이유.** 수량 `5` 가 그 줄의 `15` 안에서 발견되면
> `exact` 로 통과해 버린다. 수량·단가는 오더 금액을 직접 바꾸는 값이라
> 이 한 글자 차이가 그대로 사고가 된다. 값이 숫자로 시작·끝날 때만
> 경계를 보므로 품명 같은 문자열에는 영향이 없다.

> **날짜 표기 되돌리기가 실패하면 `miss` 로 떨구지 않고 `fuzzy`(🟡)** 로 둔다.
> 지역별 표기가 다양해서 우리가 못 만들어 본 형식이 얼마든지 있고, 멀쩡한
> 값을 🔴 로 막으면 사람이 경고를 무시하기 시작한다. `M/D` 와 `D/M` 을 둘 다
> 만드는 것도 같은 이유다 — 어느 쪽인지는 발주서마다 다르고, 우리가 단정할
> 일이 아니다.
...
> **백스톱은 수치 일치를 인정하지 않는다.** 품번은 숫자처럼 생겼어도 숫자가
> 아니다 — `1250` 과 `1,250` 은 수량이라면 같은 값이지만 품번이라면 **다른
> 품번**이고, 자릿수·앞자리 0 이 곧 의미다. 문자열로만 대조한다.

**스캔본(텍스트 레이어 없음)** 은 대조할 원문이 없다. 이때는 0·1번과 품번
백스톱을 통째로 건너뛰고 `confidence` 임계와 합계 검증만 남는다 (§3.3.6).
...
│   │   ├── schema_builder.py               OUTLINE · LINES · 단일(스캔본) 툴 스키마
...
| **앵커 대조** ★ | 숫자 경계(5 ≠ 15 안의 5) · 유럽식 수 표기 · 날짜 표기 전수 · 품번 백스톱의 문자열 엄격성 |
