...
| 공용 프로필이 모르는 값을 **추측해 채우기** | 출하처를 판매처로 넣는 식. 그럴듯하면 사람이 확인 없이 전송하고 **틀린 곳으로 오더가 간다.** 비우고 `required` 로 막는다 |
| 브랜드가 없는 고객의 업로드 허용 | 판정할 근거가 없다. `CatalogEntry.ready` 가 False 면 막는다 |
...
> ✅ **전 거래처 열기 (2026-09-18)** — 전용 규칙이 없어도 SAP 에 브랜드가
> 등록된 고객이면 `profiles/generic.yaml` 로 발주서를 읽는다. 브랜드·발주번호·
> 품번·수량까지 나오고, **모르는 값(출하처)은 비운 채 `required` 로 막는다.**
> 그럴듯한 기본값을 넣지 않은 것이 요점이다 — 넣으면 사람이 확인 없이 전송한다.
> 화면은 🟡 로 표시하고 "읽어낸 값을 반드시 확인하세요"를 띄운다.
> `validate_masters.py` 가 공용 프로필도 대표 1곳으로 검사한다.

**중요**

- **전 거래처 테스트 배포 중 — 공용 프로필로 읽는 거래처는 브랜드 매핑이 비어 있으면
  판정이 항상 실패한다.** `validate_masters.py` 가 그런 거래처를 경고로 표시한다.
  현업이 브랜드 매핑 탭에서 문구를 채우는 것이 선행 작업이다
