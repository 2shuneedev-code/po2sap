...
| 공용 프로필이 모르는 값을 **추측해 채우기** | 출하처를 판매처로 넣는 식. 그럴듯하면 사람이 확인 없이 넘긴다. **비워 두고 `required: warn`** — 빈 칸은 눈에 띈다 |
| 확정 안 된 규칙으로 **전송을 막기** | 이 프로그램의 본업은 발주서 내용을 표로 옮기는 것이다. 공용 프로필은 `warn` 만 쓴다. 거래처 규칙이 확정되면 그 파일에서 `required: true` 로 올린다 |
| 브랜드가 없는 고객의 업로드 허용 | 판정할 근거가 없다. `CatalogEntry.ready` 가 False 면 막는다 |
| 자동 생성한 매핑에 표시 생략 | 초벌은 확인 안 된 값이다. `note` 에 "자동 초벌"을 남겨 `validate_masters` 가 TODO 로 띄운다 |
...
> 등록된 고객이면 `profiles/generic.yaml` 로 발주서를 읽는다. 78곳 전부 된다.
> · **막지 않는다.** 공용 프로필은 `required: warn` 만 쓴다 — 비면 노랗게 보이되
