...
터미널도 가상환경도 몰라야 돌아가게 `.bat` 네 개를 뒀다. 폴더에서 더블클릭하면 된다.

| 파일 | 언제 | 하는 일 |
|---|---|---|
| `setup.bat` | **최초 1회** · 라이브러리 갱신 시 | 가상환경(`.venv`) 생성 → 라이브러리 설치 → `.env` 준비 |
| `check.bat` | 실행 전 · 이상할 때 | API 연결 + 마스터 규칙 점검 (**비용 0**) |
| `run.bat` | 매번 | 화면 실행. 접속 주소를 띄워 준다 |
| `run-mock-eai.bat` | 시험할 때만 | 모의 EAI 서버. **실서버 전송 시엔 쓰지 않는다** |

```
setup.bat  →  (.env 를 메모장으로 채움)  →  check.bat  →  run.bat
```

- `run.bat` 은 `0.0.0.0:8501` 로 열어 **사내 다른 PC 에서 접속**할 수 있게 한다.
  포트를 바꾸려면 `run.bat 8502`
- **창을 닫으면 화면도 꺼진다.** 서버에서는 창을 띄워 둔 채로 두거나 작업
  스케줄러에 등록한다
- 윈도우 방화벽이 처음 한 번 물어본다. **허용**해야 다른 PC 에서 보인다
- 파일은 `cp949` + CRLF 로 저장돼 있다 (한국어 윈도우 cmd 에서 한글이 깨지지
  않게). 편집할 때 UTF-8 로 저장하지 않는다 — `test_launchers.py` 가 잡는다

> `.bat` 은 리눅스 CI 가 실행해 볼 수 없다. 형식(인코딩·줄바꿈·가상환경 사용)만
> 테스트가 지키고, **실제 동작은 윈도우에서 사람이 한 번 확인해야 한다.**
...
```ini
MASTER_EDIT_PASSWORD=사내에서-정한-암호
```

| 상태 | 조회 | 편집 |
|---|---|---|
| 암호 미지정 (기본값) | ✅ | 🔒 잠김 — 안내만 뜬다 |
| 암호 지정 · 미입력 | ✅ | 🔒 암호를 넣으면 열린다 |
| 암호 입력됨 | ✅ | ✅ 그 브라우저 세션 동안 |

- **비어 있으면 잠긴다.** `EAI_ENDPOINT` 와 같은 원칙 — `.env` 를 깜빡한 채 띄웠을 때
  열려 있는 것보다 잠겨 있는 편이 안전하다
- 해제는 **브라우저 세션 단위**다. 탭을 닫으면 다시 잠긴다
- `.env` 를 고쳤으면 **화면을 다시 시작**해야 반영된다 (설정은 시작할 때 한 번 읽는다)
- 누가 고쳤는지까지 남겨야 하면 CSV 를 Git 으로 관리한다 — 화면은 "지나가다 실수로
  고치는 일"을 막을 뿐, 계정·권한 체계가 아니다

### `git pull` 로 날리지 않기 — 방어 3겹

서버에서 고친 CSV 는 그 PC 에만 있다. 그대로 두면 저장소 버전과 갈리고, 갈린 채로
`git pull` 을 하면 막힌다. **막힌 것을 푼다고 `git checkout .` 을 누르는 순간
현업이 몇 달 채운 값이 사라진다.** 세 겹으로 막는다.

**① 사본 — 항상, 아무것도 필요 없이**

저장할 때마다 덮어쓰기 **전** 내용이 `storage/master_backups/` 에 시간 도장으로
남는다. 네트워크도 Git 계정도 필요 없다. 기본 30개까지 (`MASTER_BACKUP_KEEP`).
...
```ini
MASTER_GIT_AUTOPUSH=true
```

저장할 때마다 그 CSV만 커밋·푸시한다. **켜 두면 원격이 늘 서버와 같아서 다음
`git pull` 이 그 파일을 건드릴 일이 없다** — 갈리는 상황 자체가 안 생긴다.

- 서버에 Git 자격증명(PAT·SSH 키)이 있어야 한다. 사내 보안 정책을 먼저 확인할 것
- 푸시가 실패해도 **저장과 사본은 이미 끝나 있다.** 화면이 "커밋은 됐고 푸시만
  실패"라고 구분해서 알려준다
